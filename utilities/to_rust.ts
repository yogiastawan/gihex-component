...
function readContent(file: string): string {
    let a = fs.readFileSync(file, 'utf8', (err: Error, data: string) => {
        if (err) {
...
function createStruct(va: Variable[], compName: string): string {
    if (va.length <= 0) {
        return ''
    }
...
function removeDup(src: any[]): any[] {
    return [...new Set(src)];
}

function templateRust(): string {
...
function fnName(str: string): string {
    let n = str.replace(".ejs", "");

    return n.replaceAll("-", "_");
}

function toHtml(str: string, va: Variable[], attr: Attr[]): string {
...
function createAttr(posAttr: number, offset: number, v: Variable[], attr: Attr[]): string {
...
function getVar(str: string): [Variable[], Attr[]] {

    let literal = getLiteral(str);
    let tag_open: TagVariable[] = [];
    let idx = 0;
    let idx_cl = 0;
    let tag_close: TagVariable[] = [];
    let find = 0;
...
function varGen(str: string, start: number, end: number): Variable {
    // console.log("Var: " + str);
...
function snackCase(str: string): string {
    let s: string = str;
...
function getLiteral(str: string): Attr[] {
...
function replaceRange(str: string, val: string, start: number, end: number): string {
